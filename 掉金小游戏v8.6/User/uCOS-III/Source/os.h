...
struct os_tcb {
    ...
    OS_TASK_PTR          TaskEntryAddr;                     //指向任务入口地址
    void                *TaskEntryArg;                      //参数创建时传递给任务

    OS_PEND_DATA        *PendDataTblPtr;                    //指向包含对象的列表指针
    OS_STATE             PendOn;                            //表示正在等待的任务
    ...
    OS_OBJ_QTY           PendDataTblEntries;                //要挂起的对象数组的大小
    ...
    OS_TICK              TickCtrMatch;                      //任务准备就绪的绝对时间
    ...
    CPU_TS               MsgQPendTime;                      //接收消息所需要的时间
    CPU_TS               MsgQPendTimeMax;                   //接收消息所需要的最长时间
#endif
#endif
    ...
    OS_CTX_SW_CTR        CtxSwCtr;                          //任务切换的时间
    CPU_TS               CyclesDelta;                       /* value of OS_TS_GET() - .CyclesStart                    */
    CPU_TS               CyclesStart;                       //任务恢复开始时循环计数器的快照
    OS_CYCLES            CyclesTotal;                       //任务已运行的周期数的总数
    OS_CYCLES            CyclesTotalPrev;                   //以前周期的快照

    CPU_TS               SemPendTime;                       //接收信号所需要的时间
    CPU_TS               SemPendTimeMax;                    //接收信号所需要的最长时间
#endif

#if OS_CFG_STAT_TASK_STK_CHK_EN > 0u
    CPU_STK_SIZE         StkUsed;                           //堆栈中使用的堆栈元素数量
    CPU_STK_SIZE         StkFree;                           //堆栈中剩余的堆栈元素数量
#endif

#ifdef CPU_CFG_INT_DIS_MEAS_EN
    CPU_TS               IntDisTimeMax;                     //最大关中断时间
#endif
#if OS_CFG_SCHED_LOCK_TIME_MEAS_EN > 0u
    CPU_TS               SchedLockTimeMax;                  //最大所调度器时间
#endif
...
void  		  OS_TmrSetPeriod (OS_TMR               *p_tmr,
							   OS_TICK               period);
