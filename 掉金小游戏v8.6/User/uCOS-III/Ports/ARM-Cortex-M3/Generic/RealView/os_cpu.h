...
*                                       时间戳的配置
*
* Note(s) : (1) OS_TS_GET() 通常被定义为CPU_TS_Get32（），允许CPU时间戳定时器是任何数据类型的大小。
*
*           (2) 对于提供32位或更高精度的自由运行计数器的体系结构
*               （即循环计数寄存器）
*
*               (a) 可被定义为CPU_TS_TmrRd（）中检索所述时间戳时，以提高性能。
*
*               (b) CPU_TS_TmrRd() 必须配置为大于或等于32位，以免截断。
*********************************************************************************************************
*/
