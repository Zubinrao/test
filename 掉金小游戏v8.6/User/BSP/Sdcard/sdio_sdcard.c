/**
  ...
  *          GPIO) are defined in stm32f4xx.h file, and the initialization is 
  *          performed in SD_GPIO_Init() function declared in sdio_sdcard.c 
  *          file.
...
static void SDIO_Cmd_Config(uint32_t Argument,uint32_t CmdIndex,uint32_t Response);
  
/* Private functions ---------------------------------------------------------*/

void SD_DeInit(void)								//复位SDIO接口
{
...
SD_Error SD_PowerON(void)
{
	...
	* 没有相应
	*/
	SDIO_Cmd_Config(0,SD_CMD_GO_IDLE_STATE,SDIO_Response_No);
	errorstatus = CmdError();
	if (errorstatus != SD_OK)
	{
		return(errorstatus);					//CMD响应超时(等待CMD SENT标志)
	}
	...
	/*!< CMD Response: R7 */
	SDIO_Cmd_Config(SD_CHECK_PATTERN,SDIO_SEND_IF_COND,SDIO_Response_Short);
	...
	{
		/*!< CMD55 */
		SDIO_Cmd_Config(0,SD_CMD_APP_CMD,SDIO_Response_Short);
	...
	*/  
	 ...
	 * or SD card 1.x 
	 */
	/* 如果 errorstatus = SD_CMD_RSP_TIMEOUT,则它是一个MMC卡 */
	/* 如果 errorstatus = SD_OK,则它是SD卡：SD卡2.0(电压范围不匹配)或SD卡1.x */
	if (errorstatus == SD_OK)
	{
		/*!< SD CARD */
		...
		while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
		{
			...
			SDIO_Cmd_Config(SD_VOLTAGE_WINDOW_SD | SDType,SD_CMD_SD_APP_OP_COND,SDIO_Response_Short);
...
SD_Error SD_InitializeCards(void)
{
	...
	if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
	{
		...
		SDIO_Cmd_Config(0,SD_CMD_ALL_SEND_CID,SDIO_Response_Long);
	...
	||  (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
	{
		...
		SDIO_Cmd_Config(0,SD_CMD_SET_REL_ADDR,SDIO_Response_Short);
	...
	if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
	{
		...
		SDIO_Cmd_Config((uint32_t)(rca << 16),SD_CMD_SEND_CSD,SDIO_Response_Long);
		errorstatus = CmdResp2Error();
...
SD_Error SD_SelectDeselect(uint64_t addr)
{
	...
	SDIO_Cmd_Config((uint32_t)addr,SD_CMD_SEL_DESEL_CARD,SDIO_Response_Short);
...
/** @brief  从卡中的指定地址读取一个块。
			数据传输可以通过DMA模式或轮询模式进行管理 
  * @note   此操作后应有两个功能来检查DMA控制器和SD卡状态
  *          - SD_ReadWaitOperation(): 
				此功能可以确保DMA控制器已完成所有数据的传输
  *          - SD_GetStatus(): 
				检查SD卡是否已完成数据传输并准备好数据
  * @param  readbuff: 指向包含接收缓冲区的指针
...
SD_Error SD_ReadBlock(uint8_t *readbuff, uint64_t ReadAddr, uint16_t BlockSize)
{
	...
	SDIO_Cmd_Config((uint32_t) BlockSize,SD_CMD_SET_BLOCKLEN,SDIO_Response_Short);
	...
	SDIO_Cmd_Config((uint32_t)ReadAddr,SD_CMD_READ_SINGLE_BLOCK,SDIO_Response_Short);
...
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	...
	SDIO_Cmd_Config((uint32_t) ReadAddr,SD_CMD_READ_MULT_BLOCK,SDIO_Response_Short);
...
/** @brief  该功能一直等待到SDIO DMA数据传输完成
			应SD_ReadMultiBlocks()函数之后调用，
			以确保所有数据都已由DMA控制器传输
  * @param  None.
  * @retval SD_Error: SD Card Error code.
  */
...
SD_Error SD_WriteBlock(uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize)
{
  ...
  /*!< Send CMD24 WRITE_SINGLE_BLOCK */
	SDIO_Cmd_Config((uint32_t) WriteAddr,SD_CMD_WRITE_SINGLE_BLOCK,SDIO_Response_Short);
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	...
	SDIO_Cmd_Config((uint32_t) (RCA << 16),SD_CMD_APP_CMD,SDIO_Response_Short);
	errorstatus = CmdResp1Error(SD_CMD_APP_CMD);
	if (errorstatus != SD_OK)
	{
		return(errorstatus);
	...
	/*!< To improve performance */
	SDIO_Cmd_Config((uint32_t)NumberOfBlocks,SD_CMD_SET_BLOCK_COUNT,SDIO_Response_Short);
	...
	SDIO_Cmd_Config((uint32_t)WriteAddr,SD_CMD_WRITE_MULT_BLOCK,SDIO_Response_Short);
...
/** @brief  该功能一直等到SDIO DMA数据传输完成
			应在SD_WriteBlock()和SD_WriteMultiBlocks()
			之后调用此函数
  * @param  None.
  * @retval SD_Error: SD Card Error code.
  */
...
SD_Error SD_StopTransfer(void)
{
	...
	SDIO_Cmd_Config(0,SD_CMD_STOP_TRANSMISSION,SDIO_Response_Short);
...
SD_Error SD_SendStatus(uint32_t *pcardstatus)
{
	...
	SDIO_Cmd_Config((uint32_t) RCA << 16,SD_CMD_SEND_STATUS,SDIO_Response_Short);
...
static SD_Error SDEnWideBus(FunctionalState NewState)
{
	...
	if (NewState == ENABLE)
	{
		...
		if ((scr[1] & SD_WIDE_BUS_SUPPORT) != SD_ALLZERO)
		{
			...
			SDIO_Cmd_Config(0x2,SD_CMD_APP_SD_SET_BUSWIDTH,SDIO_Response_Short);
	...
	else
	{
		...
		if ((scr[1] & SD_SINGLE_BUS_SUPPORT) != SD_ALLZERO)
		{
			...
			/*!< Send ACMD6 APP_CMD with argument as 2 for wide bus mode */
			SDIO_Cmd_Config(0,SD_CMD_APP_SD_SET_BUSWIDTH,SDIO_Response_Short);
			errorstatus = CmdResp1Error(SD_CMD_APP_SD_SET_BUSWIDTH);
			if (errorstatus != SD_OK)
			{
			return(errorstatus);
...
static SD_Error FindSCR(uint16_t rca, uint32_t *pscr)
{
	...
	/*!< Send CMD55 APP_CMD with argument as card's RCA */
	SDIO_Cmd_Config((uint32_t)8,SD_CMD_SET_BLOCKLEN,SDIO_Response_Short);
	errorstatus = CmdResp1Error(SD_CMD_SET_BLOCKLEN);
	if (errorstatus != SD_OK)
	{
		return(errorstatus);
	...
	SDIO_Cmd_Config(0,SD_CMD_SD_APP_SEND_SCR,SDIO_Response_Short);
...
/**@brief  开关高速模式
 * @note   此功能必须在 "Transfer State" 之后使用
 * @note   该操作之后应该配置PLL以使SDIOCK时钟在67-75MHz之间
 * @param  None
 * @retval SD_Error: SD Card Error code.
 */
SD_Error SD_HighSpeed (void)
{
	...
	{
		/* Set Block Size for Card */
		SDIO_Cmd_Config((uint32_t)64,SD_CMD_SET_BLOCKLEN,SDIO_Response_Short);
		errorstatus = CmdResp1Error(SD_CMD_SET_BLOCKLEN);
		if (errorstatus != SD_OK)
		{
			return(errorstatus);
		...
		SDIO_Cmd_Config(0x80FFFF01,SD_CMD_HS_SWITCH,SDIO_Response_Short);
